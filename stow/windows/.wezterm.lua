...
local function short_tab_name(tab)
  ...
  if domain:find("WSL", 1, true) then
    return "WSL"
  end
  ...
  if process == "cmd.exe" or process == "cmd" then
    return "CMD"
  end

  if process == "powershell.exe" or process == "pwsh.exe" or process == "powershell" or process == "pwsh" then
    return "PWSH"
  end

  if process == "bash.exe" or process == "bash" or process == "zsh" or process == "zsh.exe" or process == "sh" then
    return "SHELL"
  end

  if process == "wsl.exe" or process == "wslhost.exe" then
    return "WSL"
  end
...
return {
  ...
  keys = {
    ...
    { key = "phys:5", mods = "LEADER|SHIFT", action = act.SplitHorizontal { domain = "CurrentPaneDomain" } },
